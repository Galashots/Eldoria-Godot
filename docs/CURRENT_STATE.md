...
**Character-sprite polish pass (expansion backlog): done.** Upgrades Mossy the pet from a flat
placeholder `Polygon2D` blob to a real generated pixel-art sprite, per
`docs/design/EXPANSION_BACKLOG.md` and NORTH_STAR pillar 1 ("one excellent thing before a
second" — prove the polygon-to-sprite upgrade path on one actor before touching NPCs/slimes).
A new `assets/sprites/pets/gen_mossy.py` (pure Pillow, mirroring `gen_tileset.py`'s
procedural-generator precedent) renders two 24x24 idle-bob frames
(`assets/sprites/pets/mossy_idle{1,2}.png`): a rounded mint/teal body (kept in the same hue
family — ~157 degrees — as the original placeholder color, clearly separated from the grass
ramp's ~126-136 degree green), a small dark-green leaf sprout (`forest_floor` from the locked
shared palette), big friendly eyes, and — per the repo's explicit "shipped a green-on-green
invisible Mossy once" readability rule — a strong 2px near-black outline around every shape so
Mossy stays a distinct silhouette against `grass_mid` (`#339E40`) regardless of hue alone
(verified by compositing both frames over that exact hex during generation). `scenes/pets/
Pet.tscn`'s `Body` node changes from a `Polygon2D` to an `AnimatedSprite2D` with an embedded
`SpriteFrames` (a 2-frame "idle" loop, `speed = 2.5`, `autoplay`), replacing the old `Body`/
`Leaf`/`EyeLeft`/`EyeRight` polygon children with the one sprite node; the node is still named
`Body`, so `Pet.gd`'s follow AI, `GameState`'s pet stats/unlock rule, and every existing pet
test are untouched (verified — no script referenced the removed child node names). A new
isolated `tests/pet_sprite_tests.gd` (2 tests: `Body` is a `Sprite2D`/`AnimatedSprite2D` with a
...
- `scripts/pets/Pet.gd` / `scenes/pets/Pet.tscn`: follow-only pet AI (`CharacterBody2D`, no `HealthComponent`, no combat) — moves toward the player at speed 220 whenever farther than 24px away, stops inside that ring (player speed is 160). `Body` is now an `AnimatedSprite2D` (2-frame idle-bob loop) generated by `assets/sprites/pets/gen_mossy.py`; see the "Character-sprite polish pass" writeup above.
...
Runs all 19 isolated suites registered in `tests/test_runner.gd` - `tests/game_state_tests.gd`
...
`tests/pet_sprite_tests.gd` (2), `tests/combat_question_tests.gd` (6), and
`tests/prop_tests.gd` (3) -
against the real `GameState`/`AudioManager` autoloads, and prints `PASS`/`FAIL` per test plus
a summary line (**111 tests total**); exits non-zero if anything failed. See
