...
- **Diegetic session-end "rest" beat: a cozy campfire** → shipped/in-flight (treated as merged):
...
- **Region ambience pass: per-region ambient audio** → shipped/in-flight (treated as merged):
